...
static void mqtt_event_handler(void *handler_args, esp_event_base_t base, int32_t event_id, void *event_data)
{
    ...
    case MQTT_EVENT_DATA:
        ...
        else if (strncmp((const char *)event->data, "on#100#10000", event->data_len) == 0) {
            DATA = true;
            DATA_set = 3; 
        }
        break;
...
{
    if (DATA_set == 1) {
        static const uint8_t hex_data1[] = {0x48, 0x06, 0x00, 0x0c, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xFF, 0x00, 0x03, 0x00, 0x0D, 0x01, 0x79};
        size_t len1 = sizeof(hex_data1);
        uart_write_bytes(UART_NUM_0, hex_data1, len1); // 假设使用UART0进行发送
    }
    else if (DATA_set == 2) {
        static const uint8_t hex_data2[] = {0x48, 0x06, 0x00, 0x0c, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x03, 0x00, 0x0D, 0x71, 0x76};
        size_t len2 = sizeof(hex_data2);
        uart_write_bytes(UART_NUM_0, hex_data2, len2); // 假设使用UART0进行发送
    }
    else if (DATA_set == 3) {
        static const uint8_t hex_data3[] = {0x48, 0x06, 0x00, 0x0c, 0x04, 0x00, 0x00, 0x00, 0x00, 0xFF, 0x00, 0x00, 0x00, 0x03, 0x00, 0x0D, 0xFA, 0xA5};
        size_t len3 = sizeof(hex_data3);
        uart_write_bytes(UART_NUM_0, hex_data3, len3); // 假设使用UART0进行发送
    }
}
